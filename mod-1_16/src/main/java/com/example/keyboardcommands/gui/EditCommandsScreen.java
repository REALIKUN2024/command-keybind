...
public class EditCommandsScreen extends Screen {
	...
	public EditCommandsScreen(Screen lastScreen, BindingEntry binding) {
		super(new TranslatableComponent("commandkeybind.screen.edit.title", binding.getName()));
	...
	@Override
	protected void init() {
		this.nameBox = new EditBox(this.font, 0, 0, 200, INPUT_HEIGHT, TextComponent.EMPTY);
		...
		this.intervalBox = new EditBox(this.font, 0, 0, 80, INPUT_HEIGHT, TextComponent.EMPTY);
		...
		this.addButton(this.intervalBox);

		int footerY = this.height - FOOTER_HEIGHT;
		this.addButton(new Button(this.width / 2 - 110, footerY + 8, 100, 20,
			new TranslatableComponent("commandkeybind.screen.add_command"), button -> {
				this.binding.getCommands().add("");
				this.list.refreshEntries();
			}));
		this.addButton(new Button(this.width / 2 + 10, footerY + 8, 100, 20,
	...
	private void repositionElements() {
		...
		int gap = 6;
		int nameTotal = this.font.width(new TranslatableComponent("commandkeybind.screen.name_label")) + gap + this.nameBox.getWidth();
		this.nameBox.x = this.width / 2 - nameTotal / 2 + this.font.width(new TranslatableComponent("commandkeybind.screen.name_label")) + gap;
				this.nameBox.y = inputY1;
		int intervalTotal = this.font.width(new TranslatableComponent("commandkeybind.screen.interval_label")) + gap + this.intervalBox.getWidth();
		this.intervalBox.x = this.width / 2 - intervalTotal / 2 + this.font.width(new TranslatableComponent("commandkeybind.screen.interval_label")) + gap;
	...
	public void onClose() {
		if (this.lastScreen instanceof ConfigScreen) {
			((ConfigScreen) this.lastScreen).refreshEntries();
		}
	...
	private static final class CommandList extends ContainerObjectSelectionList<CommandList.Entry> {
		...
		private final class Entry extends ContainerObjectSelectionList.Entry<Entry> {
			...
			private Entry(int index) {
				this.index = index;
				this.commandBox = new EditBox(CommandList.this.minecraft.font, 0, 0, 240, 20, new TextComponent("command"));
			...
			public List<? extends GuiEventListener> children() {
				return Arrays.asList(this.commandBox, this.deleteButton);
			}
