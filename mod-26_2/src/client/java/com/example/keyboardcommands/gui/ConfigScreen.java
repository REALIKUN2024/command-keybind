...
/**
 * 主配置界面：列出所有按键绑定条目，支持添加、删除、改键、编辑指令。
 *
 * <p>全部元素使用原版控件（HeaderAndFooterLayout / ContainerObjectSelectionList / Button）。</p>
 */
public class ConfigScreen extends Screen {
	...
	public boolean keyPressed(KeyEvent event) {
		if (this.selectedKeyBinding != null) {
			if (event.isEscape()) {
				...
				this.selectedKeyBinding.setKey(InputConstants.getKey(event).getName());
			}
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
		if (this.selectedKeyBinding != null) {
			this.selectedKeyBinding.setKey(InputConstants.Type.MOUSE.getOrCreate(event.button()).getName());
			this.selectedKeyBinding = null;
			this.list.refreshEntries();
			return true;
		}
		...
		private final class Entry extends ContainerObjectSelectionList.Entry<Entry> {
			...
			private Entry(BindingEntry binding) {
				...
				this.editButton = Button.builder(Component.translatable("commandkeybind.screen.edit"), button -> {
					...
					BindingList.this.minecraft.gui.setScreen(new EditCommandsScreen(BindingList.this.screen, binding));
