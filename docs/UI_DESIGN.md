...
## 六、待确认问题（UI 相关）
1. 主界面是否包含"打开配置菜单按键"的可改键入口？（默认：不含，直接在原版 Controls 中改）
2. 删除绑定/指令是否需要二次确认？（默认：直接删除，简单）
3. 列表是否加滚动（ContainerObjectSelectionList 自带滚动，默认启用）。
4. 绑定行是否需要"启用/禁用" Checkbox？（默认：不加，保持简单）
